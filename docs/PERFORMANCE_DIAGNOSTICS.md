...
Профиль копируется; исходные файлы не изменяются. Сценарий выполняется человеком,
...
и не считается текущим PASS. Незавершённая ручная приёмка остаётся незавершённой
после экспорта.
