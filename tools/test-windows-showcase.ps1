...
$passed=$launcher.ExitCode -eq 0 -and $evidence.status -eq 'PASS' -and $evidence.mode -eq 'showcase' `
...
$result=[ordered]@{
    ...
    launcherExitCode=$launcher.ExitCode;captureCount=$captures.Count;artifactsPresent=$artifactsPresent
