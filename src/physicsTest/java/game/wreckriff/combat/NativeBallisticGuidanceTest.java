...
class NativeBallisticGuidanceTest {
    ...
    private record Result(float damage,float travelled,int explosions,long teleportGeneration,int manoeuvreTick,int firstImpactTick) {}

    @ParameterizedTest @EnumSource(VehicleDefinition.class)
    void stationaryHullIsHitButTurboCanEvadeTheTrackedSalvo(VehicleDefinition definition) {
        ...
        Result moving=salvo(definition,"turbo",false);
        assertEquals(4,moving.explosions);assertEquals(0,moving.teleportGeneration);
        assertTrue(moving.travelled>5,"The native controller must execute turbo for "+definition);
        assertTrue(moving.damage<stationary.damage*.5f,definition+" turbo damage="+moving.damage+" stationary="+stationary.damage);
    }
    @Test void sparkCanEvadeWithItsActualSpecialDash() {
        ...
        assertTrue(dash.manoeuvreTick>0&&dash.manoeuvreTick<dash.firstImpactTick,"React to the falling charges before their first impact");
        assertTrue(dash.damage<stationary.damage*.5f,"A real timed lateral dash must evade tracked artillery: damage="+dash.damage+" stationary="+stationary.damage);
    }
    @ParameterizedTest @EnumSource(VehicleDefinition.class)
    ...
    private Result salvo(VehicleDefinition definition,String manoeuvre,boolean roof) {
        ...
        try(var world=new PhysicsWorld(VEHICLES)) {
            ...
            Vector3f start=world.position(1);boolean warned=false,dashed=false;float damage=0;int manoeuvreTick=-1,firstImpactTick=-1;
            Set<Long> explosions=new HashSet<>();
            for(int tick=0;tick<960;tick++) {
                warned|=!combat.ballisticWarnings().isEmpty();
                // A dash lasts 0.35s. Reacting to the first warning ends it before later charges
                // are aimed; time this single lateral escape from the visible falling salvo instead.
                boolean dashCue=combat.ballisticWarnings().stream().anyMatch(warning->warning.remainingTicks()<=72);
                VehicleCommand movement=warned?switch(manoeuvre) {
                    case "turbo"->command(1,0,0,true,AbilityId.NONE);
                    case "dash"->dashCue&&!dashed?command(0,0,1,false,AbilityId.SPECIAL):VehicleCommand.NONE;
                    default->VehicleCommand.NONE;
                }:VehicleCommand.NONE;
                if(movement!=VehicleCommand.NONE&&manoeuvreTick<0)manoeuvreTick=tick;
                ...
                for(var event:combat.drainEvents()) {
                    if(event.type()==GameEvent.Type.EXPLOSION&&event.kind().equals("ballistic")) {
                        explosions.add(event.eventId());if(firstImpactTick<0)firstImpactTick=tick;
                    }
            ...
            Result result=new Result(damage,world.position(1).subtract(start).setY(0).length(),explosions.size(),
                    world.teleportGeneration(1),manoeuvreTick,firstImpactTick);
            System.out.printf(Locale.ROOT,"ballistic-native vehicle=%s manoeuvre=%s roof=%s result=%s%n",definition.id(),manoeuvre,roof,result);
            return result;
        }
    }
}
