    ...
    def supplies(self,counts):
        ...
        for i,spawn in enumerate(spawns):
            ...
            for p in sorted(candidates,key=lambda p:dist(spawn,p)):
                ...
                if selected:
                    a=tuple(selected[0][k]-spawn[k] for k in range(3));b=tuple(p[k]-spawn[k] for k in range(3))
                    if sum(a[k]*b[k] for k in range(3))/(dist(selected[0],spawn)*distance)>.55:continue
        ...
        self.supply_report=dict(ammunitionCounts=dict(zip(AMMO,counts)),totalAmmunition=sum(counts),spawns=len(spawns))
    def validate(self):
...
def neon():
    ...
    a.roof('shopping-passage',(710,935,670,860),18,'glass');a.wall('passage-south-front',(710,675),(935,675),18,2,'ivory');a.wall('passage-north-front',(710,855),(810,855),18,2,'ivory');a.wall('passage-east-front',(930,745),(930,855),18,2,'ivory')
    ...
    for i,(x,z) in enumerate([(700,600),(965,635),(1030,690),(590,890)]):a.building('market-corner-'+str(i),(x-30,x+30,z-24,z+24),20+i*3,'brick')
...
def carnival():
    ...
    for i,(x,z,w,d,yaw) in enumerate([(245,410,32,24,-12),(260,500,28,20,10),(375,475,40,26,-18),(205,725,34,28,15),(390,745,30,22,-8),(285,330,26,20,12),(350,650,32,24,-12)]):a.box('fair-stall-'+str(i),(x,4,z),(w,8,d),'wood' if i%2 else 'faded-red',True,yaw)
    ...
    for name,aa,bb in [('sw',(250,995),(285,975)),('se',(375,975),(430,1000)),('east-low',(445,1010),(445,1045)),('east-high',(445,1115),(415,1150)),('nw',(250,995),(235,1045)),('west-top',(235,1100),(285,1155))]:a.wall('circus-'+name,aa,bb,12,2,'faded-red')
    a.building('circus-seating-east',(382,410,995,1030),5,'ivory');a.building('circus-seating-west',(260,290,1110,1140),5,'ivory');a.building('circus-changing',(475,510,1135,1170),7,'blue')
    ...
    a.roof('ride-pavilion',(1170,1375,405,555),20,'purple');a.wall('ridehall-south',(1245,410),(1370,410),12,2,'purple');a.wall('ridehall-north',(1175,550),(1260,550),12,2,'purple');a.wall('ridehall-west',(1175,475),(1175,550),12,2,'purple');a.building('orbit-ride-core',(1250,1310,420,460),6,'steel')
    a.building('ferris-foundation',(1270,1330,680,720),3,'concrete');a.building('carousel-base',(900,950,230,280),1,'wood')
    for i,(x,z) in enumerate([(1100,500),(1380,780),(1430,650)]):a.building('ride-control-'+str(i),(x-12,x+12,z-10,z+10),6,'ivory')
    # Workshop bays, cross aisle, gantry, storage and north dispatch door.
    a.site('repair-depot','Ремонтное депо','Обслуживание аттракционов и хранение модулей','Западный ремонтный проезд → поперечный цех → северная выдача',[1140,1320,990,1120],'through-interior')
    a.roof('repair-depot',(1140,1320,990,1120),15,'steel');a.wall('depot-south',(1140,990),(1320,990),15,2,'blue');a.wall('depot-north-west',(1140,1120),(1215,1120),15,2,'blue');a.wall('depot-north-east',(1285,1120),(1320,1120),15,2,'blue')
