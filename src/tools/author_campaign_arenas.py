...
class Location:
    ...
    def portal(self,name,feet,road_start,road_end):
        # A full physical cap follows the underside of the actual sloping road.
        # Its legs stand outside the lower route; no freestanding T beside a deck.
        a,b=feet;dx,dz=b[0]-a[0],b[1]-a[1];length=math.hypot(dx,dz);nx,nz=dz/length*3,-dx/length*3
        poly=ccw([(a[0]+nx,a[1]+nz),(b[0]+nx,b[1]+nz),(b[0]-nx,b[1]-nz),(a[0]-nx,a[1]-nz)])
        start,end=self.points[road_start],self.points[road_end];rx,rz=end[0]-start[0],end[2]-start[2]
        top=lambda x,z:start[1]+(end[1]-start[1])*((x-start[0])*rx+(z-start[2])*rz)/(rx*rx+rz*rz)-1.2
        identity=self.mesh(name+'-cap',[poly],top,'cast-concrete',1)
        self.data['meshes'][-1]['thickness']=1.4
        self.data['surfaces']=[s for s in self.data['surfaces'] if s['geometryId']!=identity]
        for i,(x,z) in enumerate(feet):
            height=top(x,z)-1.4+.4;self.box(f'{name}-leg-{i}',(x,height/2,z),(5,height,5),'cast-concrete')
    ...
    def road(self,name,names,width=22,material='road-surface',kind='ROAD',object_id='',draw=True,joined=False):
        self.paths.append(dict(id=name,names=names.split(),width=width,material=material,kind=kind,objectId=object_id,draw=draw,joined=joined))
    ...
    def compile_geometry(self):
        ...
        for path in self.paths:
            ...
            for segment,(first,last) in enumerate(zip(path['names'],path['names'][1:])):
                ...
                def cross_section(index):
                    normals=[]
                    for j in (index-1,index):
                        if j<0 or j>=len(path['names'])-1:continue
                        p,q=(self.points[n] for n in path['names'][j:j+2]);xx,zz=q[0]-p[0],q[2]-p[2];ll=math.hypot(xx,zz)
                        normals.append((zz/ll,-xx/ll))
                    nx,nz=(sum(v[k] for v in normals) for k in (0,1));scale=path['width']/2/(nx*normals[0][0]+nz*normals[0][1])
                    return nx*scale,nz*scale
                start_offset=cross_section(segment) if path['joined'] else (sx,sz)
                end_offset=cross_section(segment+1) if path['joined'] else (sx,sz)
                ux,uz=start_offset;vx,vz=end_offset
                poly=ccw([(a[0]+ux,a[2]+uz),(b[0]+vx,b[2]+vz),(b[0]-vx,b[2]-vz),(a[0]-ux,a[2]-uz)])
                if path.get('endClip') and segment==len(path['names'])-2:poly=half(poly,*path['endClip'])
                flares=[]
                if segment==0 and path.get('startFlare'):
                    wide,run=path['startFlare'];nx,nz=dz/length,-dx/length;tx,tz=dx/length,dz/length
                    for sign in (-1,1):
                        flares.append(ccw([(a[0]+nx*path['width']/2*sign,a[2]+nz*path['width']/2*sign),
                            (a[0]+nx*wide/2*sign,a[2]+nz*wide/2*sign),
                            (a[0]+tx*run+nx*path['width']/2*sign,a[2]+tz*run+nz*path['width']/2*sign)]))
                ...
                if flares:
                    assert not flat and path['kind']=='RAMP'
                    pieces.extend(flares)
                ...
                level=0 if a[1]==b[1]==0 else -1 if min(a[1],b[1])<0 else 2 if path['id'].startswith('parking-') and max(a[1],b[1])>8.01 else 1
                paint=[];paint_pieces=[]
                ...
                if identity:
                    ...
                    if max(a[1],b[1])>0 and any(word in path['id'] for word in ('bridge','express','interchange','deck-service','island-','parking-')):self.data['meshes'][-1]['thickness']=1.2
                self.footprints.append((poly,a,b,path))
                self.footprints.extend((flare,a,b,path) for flare in flares)
        for name,poly,y,material,level in self.fixed:
            ...
            if name in ('island','gantry-landing-apron','parking-first-floor','parking-roof-deck','east-bridge-turning-apron'):self.data['meshes'][-1]['thickness']=1.2
            if name=='east-bridge-turning-apron':next(p for p in self.paths if p['id']=='east-lake-bridge')['geometryIds'].append(name)
    ...
    def clear(self,p,r=4):
        ...
        return True
    def supply_headroom(self,p):
        # A supported ground triangle can coexist below the start of a ramp.
        # Check the pickup footprint against every overhead driving triangle,
        # including its physical underside, rather than just finding one floor.
        for dx,dz in ((0,0),(-2,0),(2,0),(0,-2),(0,2)):
            x,z=p[0]+dx,p[2]+dz
            for mesh in self.data['meshes']:
                if not mesh['collision']:continue
                for i in range(0,len(mesh['indices']),3):
                    a,b,c=(mesh['vertices'][mesh['indices'][i+j]] for j in range(3))
                    den=(b['z']-c['z'])*(a['x']-c['x'])+(c['x']-b['x'])*(a['z']-c['z'])
                    if abs(den)<1e-8:continue
                    u=((b['z']-c['z'])*(x-c['x'])+(c['x']-b['x'])*(z-c['z']))/den
                    v=((c['z']-a['z'])*(x-c['x'])+(a['x']-c['x'])*(z-c['z']))/den
                    if min(u,v,1-u-v)<-.00001:continue
                    y=u*a['y']+v*b['y']+(1-u-v)*c['y']
                    if y>p[1]+.05 and y-mesh['thickness']<p[1]+2.8:return False
        return True
    def supplies(self,counts):
        # Socket positions and starting headings are authored data. Re-segmenting
        # a road must never silently redistribute the verified ammunition layout.
        layout=json.loads((SOURCE/'supply-layout.json').read_text(encoding='utf-8'))['arenas'][self.data['id']]
        assert [(s['id'],s['position']) for s in self.data['spawns']]==[(s['id'],s['position']) for s in layout['spawns']],self.data['id']+' changed a verified start'
        self.data['spawns']=layout['spawns'];self.data['pickups']=layout['pickups']
        actual=collections.Counter(p['type'] for p in self.data['pickups'])
        assert tuple(actual[k] for k in AMMO)==tuple(counts),(self.data['id'],'authored ammunition totals',actual)
        entrances=[point(e['position']) for boss in self.data['bosses'] for e in boss['entrances']]
        for pickup in self.data['pickups']:
            p=point(pickup['position']);self.surface_at(p)
            assert self.clear(p,2) and self.supply_headroom(p),(self.data['id'],'blocked authored socket',pickup['id'])
            assert all(dist(p,e)>5 for e in entrances),(self.data['id'],'socket under boss entry',pickup['id'])
        self.supply_report=dict(ammunitionCounts=dict(zip(AMMO,counts)),totalAmmunition=sum(counts),spawns=len(layout['spawns']),
            placementSource='src/tools/assets/arena-revision3/supply-layout.json',baselineDefinitionSha256=layout['sourceSha256'],
            verifiedBaselineAccess=layout['supplyReport']['startAccess'])
    def validate(self):
...
def excavation(a):
    # The haul-road mouths keep their verified positions and gradients. Unequal
    # corners follow the construction footprint rather than a rectangular bowl.
    outer=[(340,330),(520,330),(650,405),(650,575),(605,730),(400,730),(280,665),(280,425)]
    floor=[(390,415),(540,415),(565,445),(565,620),(530,645),(390,645),(365,610),(365,445)]
    a.holes=[outer];a.fixed=[('pit-floor',floor,-14,'district-earth',-1)]
    for i,name in [(0,'south'),(2,'east'),(4,'north'),(6,'west')]:
        poly=[outer[i],outer[i+1],floor[i+1],floor[i]]
        height={'south':lambda x,z:(z-330)*-14/85,'east':lambda x,z:(650-x)*-14/85,
                'north':lambda x,z:(730-z)*-14/85,'west':lambda x,z:(x-280)*-14/85}[name]
        a.mesh('pit-'+name+'-slope',[poly],height,'district-earth',-1)
    terraces={3:('pump-service',(587.5,642.5),-6),7:('survey-bench',(343.75,403.75),-6)}
    for i in (1,3,5,7):
        j=(i+1)%8;poly=ccw([outer[i],outer[j],floor[j],floor[i]])
        elevations={outer[i]:0,outer[j]:0,floor[j]:-14,floor[i]:-14}
        def base_height(x,z,poly=poly,elevations=elevations):
            for index in range(1,len(poly)-1):
                p,q,r=poly[0],poly[index],poly[index+1]
                den=(q[1]-r[1])*(p[0]-r[0])+(r[0]-q[0])*(p[1]-r[1])
                u=((q[1]-r[1])*(x-r[0])+(r[0]-q[0])*(z-r[1]))/den
                v=((r[1]-p[1])*(x-r[0])+(p[0]-r[0])*(z-r[1]))/den
                if min(u,v,1-u-v)>-1e-6:return u*elevations[p]+v*elevations[q]+(1-u-v)*elevations[r]
            raise ValueError(('excavation vertex outside its authored corner',x,z))
        terrace=terraces.get(i)
        if terrace:
            label,centre,level=terrace
            def height(x,z,base_height=base_height,poly=poly,centre=centre,level=level):
                # A real 24x20-metre level bench transitions back into the
                # surrounding earth; the unchanged edge heights join the ramps.
                distance=max(max(abs(x-centre[0])-12,0),max(abs(z-centre[1])-10,0))
                boundary=min(abs((q[0]-p[0])*(z-p[1])-(q[1]-p[1])*(x-p[0]))/math.dist(p,q) for p,q in zip(poly,poly[1:]+poly[:1]))
                weight=min(max(0,1-distance/12),min(1,boundary/10))
                base=base_height(x,z);return base+(level-base)*weight
            a.site('pit-'+label,label,'Рабочая земляная терраса на отметке −6 м','Боковой откос между независимыми грузовыми спусками',[centre[0]-12,centre[0]+12,centre[1]-10,centre[1]+10],'earthwork-terrace')
        else:height=base_height
        # The subdivision includes plateau edges and the original triangle seam,
        # so neither the bench nor a change of slope is approximated by a decal.
        xs=set(range(math.floor(min(x for x,z in poly)/8)*8,math.ceil(max(x for x,z in poly)/8)*8+1,8))
        zs=set(range(math.floor(min(z for x,z in poly)/8)*8,math.ceil(max(z for x,z in poly)/8)*8+1,8))
        if terrace:
            xs.update(centre[0]+d for d in (-24,-12,12,24));zs.update(centre[1]+d for d in (-22,-10,10,22))
        xs,zs=sorted(xs),sorted(zs);pieces=[]
        for x,xx in zip(xs,xs[1:]):
            for z,zz in zip(zs,zs[1:]):
                piece=poly
                clip=rect(x,xx,z,zz)
                for p,q in zip(clip,clip[1:]+clip[:1]):piece=half(piece,p,q) if piece else []
                if piece:
                    # Split at the piecewise-planar source diagonal as well.
                    for side in (True,False):
                        split=half(piece,poly[0],poly[2],side)
                        if split:pieces.append(split)
        a.mesh('pit-corner-'+str(i),pieces,height,'district-earth',-1)
    a.site('excavation','Котлован 17Б','Разные фазы фундаментных работ, дренаж и рабочие террасы','Западный, восточный, южный и северный грузовые спуски остаются независимыми',[280,650,330,730],'excavation')
...
def construction():
    ...
    excavation(a)
    a.fixed.append(('gantry-landing-apron',rect(1095,1215,965,1050),18,'concrete',1))
    ...
    a.road('interchange-rise','ramp_s deck_s',34,'cast-concrete','RAMP');a.road('interchange-span','deck_s deck_c deck_n',34,'cast-concrete');a.road('interchange-descent','deck_n ramp_n',34,'cast-concrete','RAMP')
    ...
    a.roof('unfinished-apartments',(360,595,820,950),16,'cast-concrete');a.roof('unfinished-east-wing',(555,670,900,1050),29,'cast-concrete')
    for x,z in [(370,830),(450,830),(570,810),(370,925),(450,925),(570,925),(610,940),(650,1025)]:a.box(f'frame-column-{x}-{z}',(x,8,z),(2.5,16,2.5),'cast-concrete')
    
    for x,z in [(560,910),(660,910),(560,1035),(660,1035)]:a.box(f'frame-wing-column-{x}-{z}',(x,14.5,z),(3,29,3),'cast-concrete')
    a.building('unfinished-stair-core',(550,575,870,910),42,'cast-concrete');a.building('finished-shell-west',(70,120,815,980),35,'ivory');a.building('foundation-next-phase',(235,285,1100,1150),1.2)
    ...
    for i,(x,z) in enumerate([(1020,370),(1130,375),(1350,440)]):a.building('silo-base-'+str(i),(x-17,x+17,z-17,z+17),3,'cast-concrete')
    ...
    for i,(x,z) in enumerate([(960,120),(1000,120),(1370,110),(1420,110)]):a.box('panel-stack-'+str(i),(x,2,z),(24,4,12),'cast-concrete')
    ...
    for i,(x,z) in enumerate([(1235.69,983.79),(1298.89,1038.89),(1330.71,1070.71)]):
        a.box('viaduct-pier-'+str(i),(x,7.7,z),(5,15.4,5),'cast-concrete');a.box('viaduct-pier-cap-'+str(i),(x,16.1,z),(12,1.4,8),'cast-concrete')
    a.portal('interchange-rise-portal',[(1079.05,915.79),(1116.25,863.71)],'ramp_s','deck_s')
    a.portal('interchange-descent-portal',[(1368.85,1136.68),(1389.09,1075.97)],'deck_n','ramp_n')
    a.box('gantry-apron-pier-west',(1110,8.3,970),(5,16.6,5),'cast-concrete');a.box('gantry-apron-pier-east',(1200,8.3,1040),(5,16.6,5),'cast-concrete')
...
def neon():
    ...
    a.fixed=[('underpass-floor',rect(1163,1217,540,820),-12,'road-surface',-1),('parking-first-floor',rect(1400,1660,200,500),8,'cast-concrete',1),('parking-roof-deck',rect(1400,1660,200,500),16,'cast-concrete',2)]
    ...
    a.road('parking-ramp-one','park_s p1s',24,'cast-concrete','RAMP');a.road('parking-ramp-two','p1nw p2nw',24,'cast-concrete','RAMP')
    ...
    a.roof('tunnel-cover',(1160,1220,540,820),1.5,'cast-concrete')
    a.wall('tunnel-west-wall',(1162,540),(1162,820),14,2,'cast-concrete',-12);a.wall('tunnel-east-wall',(1218,540),(1218,820),14,2,'cast-concrete',-12)
    ...
    a.roof('parking-ground-floor',(1400,1660,290,400),7.2,'cast-concrete')
    for x,z in [(1410,300),(1410,400),(1530,300),(1530,400),(1650,300),(1650,400)]:
        for level in (0,8):a.box(f'parking-column-{x}-{z}-{level}',(x,level+3.6,z),(2,7.2,2),'cast-concrete')
    for level in (8,16):
        for label,aa,bb in [('north',(1400,500),(1660,500)),('south',(1400,200),(1660,200)),('east',(1660,200),(1660,500)),('west',(1400,200),(1400,500))]:a.wall(f'parking-parapet-{level}-{label}',aa,bb,1.1,.6,'cast-concrete',level)
...
def coaster(a):
    # One named elevated ride follows a long return circuit. Road crossings are
    # overhead; columns are selected outside the complete existing road widths.
    centre=(1310,0,315)
    def track(angle):return (140*math.cos(angle),8+28*math.sin(angle/2)**2,58*math.sin(angle))
    supports=[]
    for index in range(36):
        angle=index*math.tau/36;x,y,z=track(angle)
        if a.landscape_clear(centre[0]+x,centre[2]+z,10):supports.append(dict(angle=angle,position=(x,y,z)))
    assert len(supports)>=12,('coaster needs grounded spans',len(supports))
    a.models.append(dict(id='comet-coaster',centre=centre,track=[track(i*math.tau/144) for i in range(144)],supports=supports))
    a.box('comet-coaster-proxy',(1310,20,315),(300,40,136),'steel',False)
    a.site('comet-coaster','КОМЕТА','Стальной аттракцион с подъёмом, возвратной петлёй, тормозным участком и посадкой','Посадочная станция на востоке; обслуживание со стороны существующей южной дороги; автомобильные пересечения проходят под высокими пролётами',[1160,1460,247,383],'landmark')
...
def carnival():
    ...
    # A shallow curved steel span keeps both established abutments and socket
    # positions. The complete 34-metre deck, AI corridor and bearings follow it.
    for index in range(1,8):
        t=index/8;a.points['east_curve_'+str(index)]=(870+160*t,3,650+48*t*(1-t))
    ...
    a.road('east-lake-bridge','island_e '+' '.join('east_curve_'+str(i) for i in range(1,8))+' east_high',34,'steel',joined=True);a.road('east-bridge-fall','east_high east_ramp',26,'steel','RAMP')
    # The last flat span and its turning apron meet the existing sloped ramp on
    # one exact plane. Independent perpendicular end caps left a triangular
    # hole at (1034.6,642.35), where a returning long chassis lost support.
    start,end=a.points['east_high'],a.points['east_ramp'];dx,dz=end[0]-start[0],end[2]-start[2]
    seam=[(start[0]+dz,start[2]-dx),(start[0]-dz,start[2]+dx)]
    next(p for p in a.paths if p['id']=='east-lake-bridge')['endClip']=seam
    next(p for p in a.paths if p['id']=='east-lake-bridge')['junctionEnd']=40
    next(p for p in a.paths if p['id']=='east-bridge-fall')['startFlare']=(60,20)
    apron=half(rect(990,1050,620,686),*seam)
    a.fixed.append(('east-bridge-turning-apron',apron,3,'steel',1))
    a.points.update(east_turn_s=(1028,3,633),east_turn_w=(1008,3,633))
    # This connection follows the actual supported abutment apron. Its explicit
    # lateral turn keeps an inspection drive from requesting a 160m reverse.
    a.road('east-abutment-turn','east_high east_turn_s east_turn_w east_curve_7',12,'steel',draw=False)
    for i,(x,z) in enumerate([(1000,630),(1008,675),(1040,622)]):
        base=-18 if inside((x,z),lake) else 0
        a.box('east-abutment-support-'+str(i),(x,(base+1.8)/2,z),(3,1.8-base,3),'cast-concrete')
    a.road('north-lake-bridge','island_n north_high',24,'concrete');a.road('north-bridge-fall','north_high north_ramp',24,'concrete','RAMP')
    # Each bridge has its own bearing construction, founded in the authored bed
    # at -18m. Capitals overlap the exact 1.8m deck underside by five centimetres.
    for i,x in enumerate((550,585,620)):
        for side in (-1,1):a.box(f'west-bridge-pile-{i}-{side}',(x,-8.45,650+side*10),(2,19.5,2),'wood')
        a.box(f'west-bridge-cap-{i}',(x,1.55,650),(5,.6,29),'wood')
    for name,positions,cap,material in [
        ('east',[(x,650+48*((x-870)/160)*(1-(x-870)/160)) for x in (900,940,980)],(7,.6,33),'steel'),
        ('north',[(755,835),(755,870),(755,900)],(23,.6,7),'cast-concrete')]:
        for i,(x,z) in enumerate(positions):
            a.box(f'{name}-bridge-pier-{i}',(x,-8.45,z),(4,19.5,4),material)
            tangent=48*(1-2*(x-870)/160)/160 if name=='east' else 0
            a.box(f'{name}-bridge-cap-{i}',(x,1.55,z),cap,material,yaw=-math.degrees(math.atan(tangent)))
...
def expand_yard():
    ...
    next(p for p in data['pickups'] if p['id']=='additional-ballistic_ammo-22')['position']=vec((52,0,46))
    ...
    art_path=OUT/'arena-art-dead-air-yard.json'
    art=json.loads(art_path.read_text(encoding='utf-8'))
    for group in art['groups']:group.setdefault('radius',0)
    write_text_atomic(art_path,json.dumps(art,ensure_ascii=False,separators=(',',':'))+'\n')
...
def write_review_routes():
    import html
    specs={
        'construction_17': [('pit','district','west pit_w pit_wb pit pit_eb pit_e'),('homes','district','west frame_w nw north frame_out frame_e'),('plant','district','spine plant_w plant_in plant_s silos_s warehouse_w'),('warehouses','district','south warehouse_w silos_s plant_s warehouse_out warehouse_e'),('interchange','district','ramp_s deck_s deck_c deck_n ramp_n'),('unfinished-apartments','interior','frame_w frame_in frame_turn frame_out'),('concrete-plant','interior','plant_w plant_in plant_turn plant_n'),('warehouse','interior','warehouse_w warehouse_in warehouse_turn warehouse_out')],
        'neon_zero': [('business','district','business_w business_c business_e n'),('market','district','market_w alley_a home_e market_s'),('homes','district','home_w home_c home_e s'),('transport','district','tech_e tn tnb tc tsb ts'),('parking','district','park_w park_c park_align park_gate park_s p1s p1e p1ne p1north p1west p1align p1nw p2nw'),('shopping-passage','interior','market_w market_in market_c market_n'),('technical-complex','interior','tech_w tech_in tech_c tech_e'),('parking-ground-floor','interior','park_w park_c park_e park_out')],
        'euphoria_park': [('entrance','district','entry entry_n fair_s'),('fair','district','fair_s fair_c fair_e shore_w'),('lake','district','west_ramp west_high island_w island_c island_e east_curve_1 east_curve_2 east_curve_3 east_curve_4 east_curve_5 east_curve_6 east_curve_7 east_high east_turn_s east_turn_w east_curve_7 east_curve_6 east_curve_5 east_curve_4 east_curve_3 east_curve_2 east_curve_1 island_e island_n north_high north_ramp'),('rides','district','rides_n rides_c ride_out se'),('circus','district','gate_a circus_w circus_n circus_out'),('backstage','district','north depot_out depot_e backstage'),('circus','interior','circus_s circus_in circus_e circus_out'),('ride-pavilion','interior','rides_s ride_in ride_turn ride_out'),('repair-depot','interior','depot_w depot_in depot_turn depot_out')]}
    routes=[]
    for factory in (construction,neon,carnival):
        ...
        connections={frozenset((first,last)) for path in a.paths for first,last in zip(path['names'],path['names'][1:])}
        seen=set()
        for name,kind,names in specs[identity]:
            nodes=names.split()
            assert all(frozenset(pair) in connections for pair in zip(nodes,nodes[1:])),(identity,name,'review leaves authored roads')
            signature=tuple(nodes)
            assert signature not in seen,(identity,name,'duplicate district/interior review')
            seen.add(signature)
            routes.append(dict(arenaId=identity,id=name,kind=kind,points=[vec(a.points[n]) for n in nodes]))
