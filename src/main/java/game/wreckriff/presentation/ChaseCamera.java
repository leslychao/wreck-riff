...
public final class ChaseCamera {
    public static final float NEAR_PLANE=.3f;
    ...
    public void update(PhysicsWorld world,int id,float alpha,float dt,boolean rear,boolean turbo,float intensity) {
        ...
        float aspect=camera.getWidth()/(float)camera.getHeight();
        // Enclose the entire near rectangle, including its forward offset from the eye.
        // This sphere also remains valid after camera pitch and the final roll shake.
        float radius=Math.max(rules.sweepRadius(),nearPlaneRadius(fov,aspect));
        var collision=world.staticSweep(pivot,desired,radius);
        ...
        if(!initialized) { position.set(desired); target.set(aim); initialized=true; }
            ...
            var finalCollision=world.staticSweep(pivot,candidate,radius);
        ...
        camera.setFrustumPerspective(fov,aspect,NEAR_PLANE,farDistance);
        ...
        Vector3f shaken=position.add(offset);
        var shakeCollision=world.staticSweep(pivot,shaken,radius);
        if(shakeCollision!=null)shaken=pivot.clone().interpolateLocal(shaken,
                Math.max(0,shakeCollision.fraction()-rules.wallMargin()/Math.max(.1f,pivot.distance(shaken))));
        camera.setLocation(shaken); camera.lookAt(target,Vector3f.UNIT_Y);
    ...
    static float nearPlaneRadius(float fov,float aspect) {
        float halfHeight=NEAR_PLANE*FastMath.tan(fov*FastMath.DEG_TO_RAD*.5f);
        return (float)Math.sqrt(NEAR_PLANE*NEAR_PLANE+halfHeight*halfHeight*(1+aspect*aspect));
    }
