...
final class WeaponMountVisual {
    ...
    void accept(GameEvent event) {
        if(event.type()!=GameEvent.Type.SHOT||event.emission()==null)return;
        int index=switch(event.emission().socketId()){case "machine-gun-muzzle-0"->0;case "machine-gun-muzzle-1"->1;case "weapon-muzzle"->2;default->-1;};
        if(index<0)return;
        ...
        // Delivery happens after the frame's advance: show the shot now, before the next elapsed interval.
        bind();applyPose(index);
    }
    ...
    private void applyPose(int index){float travel=recoil[index]*(index<2?.055f:rocket?.012f:.15f);for(Geometry bolt:bolts.get(index))bolt.setLocalTranslation(0,0,-travel);for(Geometry barrel:barrels.get(index)){barrel.setLocalTranslation(0,0,-travel*(index<2?.12f:rocket?.12f:1));barrel.getMaterial().setFloat("Heat",heat[index]);}}
