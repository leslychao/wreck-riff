...
public final class FrameMetrics {
    ...
    private long count,over33ms,over50ms,over100ms,histogramOverflowFrames;
    ...
    public void add(double frameSeconds) {
        ...
        if(milliseconds>1000)histogramOverflowFrames++;
    }
    ...
    private double percentile(double fraction) {
        ...
        for(int i=0;i<histogram.length;i++) { sum+=histogram[i];if(sum>=threshold)return i==histogram.length-1?Math.max(1000,maximum):i/20.0; }
        return 0;
    }
    public Map<String,Object> snapshot() {
        Map<String,Object> data=new LinkedHashMap<>();
        data.put("frames",count);data.put("sampleSeconds",seconds);data.put("histogramPrecisionMs",0.05);
        data.put("histogramOverflowFrames",histogramOverflowFrames);data.put("percentilesAbove1000msAreUpperBounds",true);
        if(count>0) {
