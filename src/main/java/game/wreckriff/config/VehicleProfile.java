...
public final class VehicleProfile {
    ...
    private final float length,width,height,mass,speedMultiplier,accelerationMultiplier,turnMultiplier;
    ...
    private VehicleProfile(String id,VehicleRules reference,float length,float width,float height,
        ...
        this.speedMultiplier=speedMultiplier;this.accelerationMultiplier=accelerationMultiplier;this.turnMultiplier=turnMultiplier;
        ...
        mass=reference.mass()*scaleX*scaleZ;
        hullBoxes=List.of(new HullBox(0,.2f*scaleY,0,width/2,.3f*scaleY,length/2),
                new HullBox(0,.65f*scaleY,-.25f*scaleZ,.83f*scaleX,.32f*scaleY,.85f*scaleZ));
        hullBounds=new Envelope(-width/2,width/2,-.1f*scaleY,.97f*scaleY,-length/2,length/2);
        ...
        fullBounds=new Envelope(-Math.max(width/2,wheelX),Math.max(width/2,wheelX),
                ...
                Math.max(hullBounds.maxY(),wheelConnection(0).y+wheelRadius()),
    ...
    public Vector3f weaponBase() { return new Vector3f(0,.55f*scaleY,1.6f*scaleZ); }
    public Vector3f muzzle() { return new Vector3f(0,.55f*scaleY,2.5f*scaleZ); }
