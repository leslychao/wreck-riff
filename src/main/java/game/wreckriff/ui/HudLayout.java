...
public record HudLayout(int width,int height,float scale,float fontSize,int weaponColumns,
    ...
    public static HudLayout compute(int width,int height,float requestedScale) {
        ...
        float abilityWidth=3*slotWidth+gap*2+pad*2;
        ...
        if(abilities.x()<health.right()+gap)abilities=new UiBounds(margin,health.top()+gap,abilityWidth,slotHeight+pad*2);
        ...
        UiBounds notification=new UiBounds(margin,Math.max(abilities.top(),Math.max(health.top(),weapons.top()))+Math.max(margin,40*scale),
                Math.min(620*scale,width/2f-margin-34*scale),Math.max(124,140*scale));
        ...
        List<UiBounds> abilitySlots=List.of(new UiBounds(abilities.x()+pad,abilities.y()+pad,slotWidth,slotHeight),
                ...
                new UiBounds(abilities.x()+pad+2*(slotWidth+gap),abilities.y()+pad,slotWidth,slotHeight));
