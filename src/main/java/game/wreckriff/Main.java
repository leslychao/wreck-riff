...
public final class Main {
    public record Options(boolean dev,long seed,boolean fixedSeed,boolean noAudio,boolean aiPlayer,int smokeSeconds,Path configDir,int benchmarkSeconds) {
        public boolean automated() { return smokeSeconds>0 || benchmarkSeconds>0; }
        public static Options parse(String[] args) {
            boolean dev=false,fixed=false,mute=false,ai=false; long seed=System.nanoTime(); int smoke=0,benchmark=0; Path config=null;
            for(String arg:args) {
            ...
            if(!dev && (fixed||mute||ai||smoke>0||benchmark>0||config!=null)) throw new IllegalArgumentException("Diagnostic options require --dev");
            if(smoke>0 && benchmark>0) throw new IllegalArgumentException("Choose smoke or benchmark mode");
            return new Options(dev,seed,fixed,mute,ai,smoke,config,benchmark);
        }
    }
    public static void main(String[] args) {
        if(args.length==1&&args[0].equals("--help")) { System.out.println("Wreck Riff: --dev [--seed=<long>] [--no-audio] [--ai-player] [--smoke-seconds=1..3600] [--benchmark-seconds=1..3600] [--config-dir=<path>]"); return; }
        try {
            ...
            settings.setTitle("Wreck Riff | 0.2.0"); settings.setResolution(user.width,user.height);
