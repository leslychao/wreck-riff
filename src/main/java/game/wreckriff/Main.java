...
public final class Main {
    public record Options(boolean dev,long seed,boolean fixedSeed,boolean noAudio,boolean aiPlayer,int smokeSeconds,Path configDir,int benchmarkSeconds,boolean showcase,
        ...
        public boolean automated() { return smokeSeconds>0 || benchmarkSeconds>0 || soakSeconds>0 || showcase || artShowcase || vehicleShowcase; }
        public static Options parse(String[] args) {
            ...
            long seed=System.nanoTime();int smoke=0,benchmark=0,soak=0,resolution=0;Path config=null;String arena="dead-air-yard";
            for(String arg:args) {
            ...
            if(!dev && (fixed||mute||ai||smoke>0||benchmark>0||soak>0||showcase||art||vehicles||arenaSpecified||resolution>0||noGlow||profile||config!=null)) throw new IllegalArgumentException("Diagnostic options require --dev");
            if((smoke>0?1:0)+(benchmark>0?1:0)+(soak>0?1:0)+(showcase?1:0)+(art?1:0)+(vehicles?1:0)>1) throw new IllegalArgumentException("Choose one diagnostic mode");
            ...
            return new Options(dev,seed,fixed,mute,ai,smoke,config,benchmark,showcase,arena,art,resolution,noGlow,profile,vehicles,soak);
        }
    }
    public static void main(String[] args) {
        if(args.length==1&&args[0].equals("--help")) { System.out.println("Wreck Riff: --dev [--seed=<long>] [--no-audio] [--ai-player] [--arena=<stable-id>] [--resolution=720p|1080p] [--no-glow] [--profile] [--smoke-seconds=1..3600 | --benchmark-seconds=1..3600 | --soak-seconds=1..3600 | --showcase | --art-showcase | --vehicle-showcase] [--config-dir=<path>]"); return; }
        try {
